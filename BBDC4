...
train = BBDC4_df['2008-04-01':'2008-04-23']
validation = BBDC4_df['2008-04-24':]
...
# BBDC4_dfのデータを標準化します
BBDC4_scaled = scaler.fit_transform(BBDC4_df)

# 標準化したデータを再びデータフレームに変換します
BBDC4_scaled_df = pd.DataFrame(BBDC4_scaled, columns=BBDC4_df.columns, index=BBDC4_df.index)

# 指定された日付でデータを訓練データと検証データに分割します
train_data = BBDC4_scaled_df['2008-04-01':'2008-04-23']
validation_data = BBDC4_scaled_df['2008-04-24':]
...
dummy_features = np.zeros(shape=(len(validation_forecast), BBDC4_scaled_df.shape[1] - 1))
...
# 指定された日付でデータを訓練データと検証データに分割します
train = BBDC4_scaled_df['2008-04-01':'2008-04-23']
validation = BBDC4_scaled_df['2008-04-24':]
...
# プロットのタイトルと軸ラベルを設定します
fig.update_layout(title='BBDC4 Predicts vs Actual',
                  xaxis_title='Date',
                  yaxis_title='Value')
