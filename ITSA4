...
train = ITSA4_df['2008-04-01':'2008-04-23']
validation = ITSA4_df['2008-04-24':]
...
# ITSA4_dfのデータを標準化します
ITSA4_scaled = scaler.fit_transform(ITSA4_df)

# 標準化したデータを再びデータフレームに変換します
ITSA4_scaled_df = pd.DataFrame(ITSA4_scaled, columns=ITSA4_df.columns, index=ITSA4_df.index)

# 指定された日付でデータを訓練データと検証データに分割します
train_data = ITSA4_scaled_df['2008-04-01':'2008-04-23']
validation_data = ITSA4_scaled_df['2008-04-24':]
...
# 指定された日付でデータを訓練データと検証データに分割します
train = ITSA4_scaled_df['2008-04-01':'2008-04-23']
validation = ITSA4_scaled_df['2008-04-24':]
...
# プロットのタイトルと軸ラベルを設定します
fig.update_layout(title='ITSA4 Predicts vs Actual',
                  xaxis_title='Date',
                  yaxis_title='Value')
